...
def chat():
    ...
    # Use AI to process
    response = process_with_ai(context, message)
...
def settings():
    ...
    if request.method == 'GET':
        ...
        settings = {
            'learning_algorithm': os.environ.get('LEARNING_ALGO', 'markov'),
...
def get_recent_learnings(limit=5):
    ...
    return '\n'.join([f"- {l['topic']}: {l['content'][:100]}" for l in learnings])

def process_with_ai(context, message):
    """Process message with AI from scratch"""
    if ai:
        return ai.generate_response(message)
